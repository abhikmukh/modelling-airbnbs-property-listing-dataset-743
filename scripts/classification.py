...
def classification_hyper_tune(features: pd.DataFrame, label: pd.DataFrame) -> float:
    ...
    print(f"Base line model precision score {precision_score(y_test, y_predictions, average='weighted')}")
    print(f"Base line model recall score {recall_score(y_test, y_predictions, average='weighted')}")
    print(f"Base line model f1 score {f1_score(y_test, y_predictions, average='weighted')}")
    ...
    results = modelling_utils.evaluate_all_models(ml_method="sk_learn_ml", list_of_models=models,
                                                  param_file=hyperparameter_file, X_train=X_train, y_train=y_train,
